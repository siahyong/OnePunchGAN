...
class Finn(object):
    ...
    def train(self, config):
        ...
        for epoch in range(config.epoch):
            ...
            if np.mod(epoch, 5) == 0:
                ...
                print('Saving images...')
                [ imwrite(os.path.join(config.image_dir,"G_epoch%dimg%d.jpg" %
                ...
                if(epoch == 0):
                    ...
                    [ imwrite(os.path.join(config.image_dir,"Z13_epoch%dimg%d.jpg" %
                    ...
                    [ imwrite(os.path.join(config.image_dir,"Z2_epoch%dimg%d.jpg" %
    ...
    def test(self, config):
        ...
        print('Saving images...')
        [ imwrite(os.path.join(config.image_dir,"G_valimageno%d.jpg" %
                 (i)), np.squeeze(G_img[i])) for i in range(G_img.shape[0]) ]


        Z_imgs = self.val_doublets
        [ imwrite(os.path.join(config.image_dir,"Z13_valimgno%d.jpg" %
                     (i)), (Z_imgs[i,:,:,:3] + Z_imgs[i,:,:,3:])/2 + self.mean_img) for i in range(Z_imgs.shape[0]) ]

        Z_imgs = self.val_targets
        [ imwrite(os.path.join(config.image_dir,"Z2_valimagno%d.jpg" %
